...
Coolify: https://dashboard.duomnia.com/ → root team → projects → Printy

- to deploy backend: → backend → deployments → redeploy
- to manually run the job that prints tasks: → backend → configuration → scheduled tasks
